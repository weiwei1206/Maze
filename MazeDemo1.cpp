...
#define MAX_ROW 10
#define MAX_COL 10
enum MAZE_MOVE { Up = 0, Down = 1, Left = 2, Right = 3 };
enum MAZE_WALL { NoWall = 0, Wall = 1 };
enum MAZE_CELL { Blank = 0, Start = 1, End = 2, Block = 3 };

int distance[MAX_ROW * MAX_COL] = { 0 };
int former[MAX_ROW * MAX_COL] = {0};
int visited[MAX_ROW * MAX_COL] = {0};
MAZE_WALL wall[] = { NoWall, NoWall ,Wall,Wall,Wall,NoWall ,NoWall ,Wall,NoWall ,NoWall ,NoWall ,Wall,NoWall ,Wall,NoWall ,NoWall ,NoWall ,Wall,NoWall ,Wall,NoWall ,NoWall ,NoWall ,Wall };//设计好的迷宫
int row = 4;//先用一个4*4的迷宫练练手
int col = 4;
int start = 1;
int end = 13;
...
COMPLETE:
	...
	std::cout << "路径长度：" << distance[end] << std::endl;
}